 ...
 * <LogoIcon />    — circular icon (horse + P jockey)
 *
 * Horse path source: SVG Repo (svgrepo.com), free licence.
 * Original viewBox: 0 0 219.175 219.175
 *
 * The bold amber "P" sits on the horse's back as the jockey.
 * If position needs nudging, adjust JOCKEY_X / JOCKEY_Y below.
 */

const HORSE_PATH =
  "M196.904,41.306c0.665-0.744,1.361-1.681,2.225-2.435c0.751-0.66,1.111-1.741,1.858-2.401" +
...
export function Logo({ white = false, className = "", height = 32 }: LogoProps) {
  ...
  return (
    <svg
      ...
      style={{ display: "block" }}
    >
      {/* Horse silhouette scaled to fit ~52px tall */}
      <g transform={`translate(4,8) scale(${WM_SCALE})`} fill={fg}>
        <path d={HORSE_PATH} />
      </g>
...
export function LogoIcon({ size = 40, className = "" }: LogoIconProps) {
  return (
    ...
    >
      ...
      {/* Horse silhouette */}
      <g transform="translate(75,50) scale(1.65)" fill="white">
        <path d={HORSE_PATH} />
      </g>
